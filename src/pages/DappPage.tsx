  ...
  return (
    <div className="premium-surface overflow-hidden rounded-2xl">
      ...
      {latestReceipts.length > 0 ? (
        ...
        <div className="flex min-h-[92px] items-center gap-4 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-5 py-5 text-frost-dim">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[var(--surface-border)] bg-[var(--surface-raised)]">
            <FileText size={17} />
          </div>
          <p className="text-base leading-6">
            {connected ? "New sealed files for this wallet appear here." : "Connect a wallet to load vault activity for that account."}
          </p>
        </div>
      )}
      </div>
...
function ActivityPanel({
  ...
  return (
    <Panel title="Vault activity" eyebrow="audit log">
      <div className="mb-5 grid gap-4 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <p className="max-w-3xl text-sm leading-6 text-frost-dim">
            A wallet-scoped activity log for file registration, sharing, revocation, deletion, and recovery operations.
          </p>
          {accountAddress && (
      ...
      {!connected ? (
        <div className="flex min-h-[220px] flex-col items-center justify-center rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-6 py-12 text-center">
          <Wallet size={24} className="mb-3 text-frost-muted" />
          <p className="font-display text-lg font-semibold text-frost">Connect wallet to load activity</p>
          <p className="mt-2 max-w-sm text-sm leading-6 text-frost-dim">
            Activity is separated by wallet and Shelby network.
          </p>
        </div>
      ) : rows.length === 0 ? (
        <div className="flex min-h-[220px] flex-col items-center justify-center rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-6 py-12 text-center">
          <Clock size={24} className="mb-3 text-frost-muted" />
          <p className="font-display text-lg font-semibold text-frost">No activity recorded yet</p>
          <p className="mt-2 max-w-sm text-sm leading-6 text-frost-dim">
            Seal a file, grant access, or save a recovery point to start the audit log.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {pageRows.map((row) => (
            <div key={row.id} className="grid gap-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4 transition-colors hover:border-[var(--surface-border-strong)] hover:bg-[var(--soft-hover)] lg:grid-cols-[minmax(0,1fr)_auto]">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-display text-base font-semibold text-frost">{row.title}</p>
                  <span className="rounded-md border border-[var(--surface-border)] bg-[var(--surface-raised)] px-2 py-1 font-mono text-[10px] uppercase tracking-[0.1em] text-frost-muted">
                    {row.source}
                  </span>
                </div>
                <p className="mt-2 text-sm leading-6 text-frost-dim">{row.copy}</p>
                {row.blobName && (
                  <p className="mt-2 max-w-4xl truncate rounded-lg border border-[var(--surface-border)] bg-[var(--surface-raised)] px-3 py-2 font-mono text-[11px] text-frost-muted">
                    {row.blobName}
                  </p>
                )}
              </div>
              <div className="flex flex-col gap-2 lg:items-end">
                <span className="font-mono text-[11px] text-frost-muted">{new Date(row.createdAt).toLocaleString()}</span>
                {row.txHash && (
                  <a
          ...
          {totalPages > 1 && (
            <div className="flex flex-col gap-3 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
  ...
  return (
    <div className="space-y-5">
      <section className="premium-surface rounded-2xl p-5 md:p-6">
        ...
        <details className="mt-4 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)]">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-1 font-display text-base font-semibold text-frost">
            {loading ? "Verifying access registry..." : runtime?.details ?? "Access registry status is not available."}
          </p>
        ...
        <div className="grid gap-2 sm:grid-cols-2 xl:min-w-[520px] xl:grid-cols-4">
          {items.map(([label, ready]) => (
            <div key={label} className="rounded-lg border border-[var(--surface-border)] bg-[var(--surface-raised)] px-3 py-2">
              <p className="crisp-label font-mono uppercase">{label}</p>
              <p className={`mt-1 font-display text-sm font-semibold ${
                loading ? "text-frost-muted" : ready ? "text-[var(--acid)]" : "text-danger"
              }`}>
                {loading ? "checking" : ready ? "ready" : "needs init"}
              </p>
            </div>
          ))}
        </div>
...
for item in s3.list_objects_v2(Bucket="${bucket}", Prefix="blobsafe/encrypted/").get("Contents", []):
  ...
  return (
    <section className="premium-surface rounded-2xl p-5 md:p-6" data-testid="s3-gateway-guide">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-frost-dim">
            Point S3-compatible tools at the same Shelby namespace after files are sealed by BlobSafe.
          </p>
        </div>
        <a
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {rows.map(([label, value]) => (
          <div key={label} className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-4 py-3">
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-frost-muted">{label}</p>
            <p className="mt-1 break-all font-mono text-xs leading-5 text-frost">{value}</p>
          </div>
        ))}
      ...
      <div className="mt-4 grid gap-3 rounded-xl border border-[var(--surface-border-strong)] bg-[var(--acid-glow)] px-4 py-4 md:grid-cols-[160px_1fr]">
...
function S3Snippet({
  ...
  return (
    <div className="group overflow-hidden rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] transition-colors duration-200 hover:border-[var(--surface-border-strong)]">
      <div className="flex items-center justify-between gap-3 border-b border-[var(--surface-border)] px-4 py-3">
        ...
        <button
          ...
          className="themed-secondary inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl px-3 font-mono text-[11px] uppercase tracking-[0.08em] focus:outline-none focus:ring-2 focus:ring-acid/30"
        >
          <Copy size={13} />
          {copied ? "Copied" : "Copy"}
        </button>
