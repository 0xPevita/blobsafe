...
export function LandingPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  return (
    <main className="landing-page relative z-10 overflow-hidden">
      <div className="landing-ambient-orbit pointer-events-none absolute left-[50%] top-[14%] h-[34rem] w-[34rem] -translate-x-1/2 rounded-full" />
      <section className="mx-auto grid min-h-[calc(100dvh-4rem)] max-w-[1760px] grid-cols-1 gap-12 px-4 pb-16 pt-10 md:grid-cols-[1.08fr_0.92fr] md:px-6 md:pb-20 md:pt-12 lg:gap-16 2xl:px-8">
        <div className="flex flex-col justify-center">
          ...
          <h1 className="motion-stagger motion-delay-2 max-w-[12.4ch] font-display text-[clamp(3rem,7.4vw,7.2rem)] font-semibold leading-[0.9] tracking-[-0.055em] text-frost">
            <span className="landing-title-line">Wallet-owned</span>
            <span className="landing-title-line landing-title-line-delayed">private storage.</span>
          </h1>
          ...
          <div className="motion-stagger motion-delay-4 mt-9 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className="landing-cta premium-button group inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-5 py-3 font-display text-base font-semibold shadow-[0_18px_40px_-28px_rgba(156,206,118,0.75)] focus:outline-none focus:ring-2 focus:ring-acid/50 focus:ring-offset-2 focus:ring-offset-obsidian-950"
            >
              Open vault
            ...
            <a
              href="#protocol"
              className="landing-cta-subtle themed-secondary inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-5 py-3 font-display text-base font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-acid/40 focus:ring-offset-2 focus:ring-offset-obsidian-950"
            >
          ...
          <div className="grid gap-px overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-border)] sm:grid-cols-3">
            {proofPoints.map((item, index) => {
              ...
              return (
                <article
                  key={item.title}
                  className="protocol-card group bg-[var(--surface)] p-6 transition-colors duration-200 hover:bg-[var(--surface-raised)] md:p-8"
      ...
      <section className="mx-auto max-w-[1760px] px-4 py-16 md:px-6 md:py-20 2xl:px-8">
        ...
        <div className="premium-surface mt-5 grid gap-8 rounded-2xl p-6 md:grid-cols-[1fr_auto] md:items-center md:p-8">
          ...
          <button
            type="button"
            onClick={() => onNavigate("/app")}
            className="landing-cta-subtle themed-secondary group inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-5 py-3 font-display text-base font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-acid/40 focus:ring-offset-2 focus:ring-offset-obsidian-950"
          >
            Open vault
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-0.5" />
          </button>
  ...
  return (
    <div className="motion-stagger motion-delay-5 mt-10 grid gap-3 sm:grid-cols-3">
      {checks.map((check, index) => (
        <div
          key={check}
          className="data-ribbon-item group flex min-h-12 items-center gap-2 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-3 transition-colors hover:border-[var(--surface-border-strong)] hover:bg-[var(--soft-hover)]"
    ...
    <div className="grid gap-px overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-border)] sm:grid-cols-2 xl:grid-cols-5">
      {trustSignals.map((signal, index) => {
        ...
        return (
          <div
            key={signal.label}
            className="trust-signal-card group bg-[var(--surface)] px-4 py-5 transition-colors duration-200 hover:bg-[var(--surface-raised)]"
  ...
  return (
    <div className="motion-stagger motion-delay-3 w-full max-w-[680px]">
      <div className="premium-surface landing-preview relative rounded-[18px] p-3">
        <div className="landing-preview-scan pointer-events-none absolute inset-x-3 top-3 z-10 h-px" />
          ...
          <div className="grid gap-px bg-[var(--surface-border)] md:grid-cols-4">
            {pipeline.map((step, index) => {
              ...
              return (
                <div
                  key={step.label}
                  className="pipeline-preview-step group bg-[var(--surface-raised)] p-4"
                ...
                >
                  ...
                  <p className="mt-5 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-frost-muted">
                    <span className="pipeline-preview-dot h-1.5 w-1.5 rounded-full bg-[var(--acid)]" />
                    {step.label}
                  </p>
