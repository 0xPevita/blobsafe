...
import { Check, ChevronLeft, ChevronRight, Folder, FolderOpen, MoreHorizontal, Pencil, Plus, Trash2, X } from "lucide-react";
...
export function FolderRail({ blobNames = [], compact = false, accountAddress }: FolderRailProps) {
  ...
  const totalFolderPages = Math.max(1, Math.ceil(visibleFolders.length / FOLDERS_PER_PAGE));
  const folderPageStart = folderPage * FOLDERS_PER_PAGE;
  const folderPageEnd = Math.min(folderPageStart + FOLDERS_PER_PAGE, visibleFolders.length);
  const paginatedFolders = useMemo(
    () => visibleFolders.slice(folderPageStart, folderPageEnd),
    [folderPageEnd, folderPageStart, visibleFolders]
  );

  useEffect(() => {
    setFolderPage((current) => Math.min(current, totalFolderPages - 1));
  }, [totalFolderPages]);

  useEffect(() => {
    const activeIndex = visibleFolders.findIndex((folder) => folder.path === activeFolder);
    if (activeIndex === -1) return;
    setFolderPage(Math.floor(activeIndex / FOLDERS_PER_PAGE));
  ...
  const goToFolderPage = (nextPage: number) => {
    setMenuPath(null);
    setRenamingPath(null);
    setFolderPage(Math.min(Math.max(nextPage, 0), totalFolderPages - 1));
  };
  ...
  return (
    <section className={`folder-rail premium-surface rounded-2xl ${menuPath ? "is-menu-open" : ""} ${compact ? "p-4" : "p-5"}`}>
      ...
      {visibleFolders.length > FOLDERS_PER_PAGE && (
        <div className="folder-pagination mt-4 flex flex-col gap-3 rounded-xl px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-frost-muted">
              folders
            </p>
            <p className="mt-0.5 font-display text-sm font-semibold text-frost">
              {folderPageStart + 1}-{folderPageEnd} of {visibleFolders.length}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => goToFolderPage(folderPage - 1)}
              disabled={folderPage === 0}
              className="folder-page-button inline-flex min-h-10 min-w-10 items-center justify-center rounded-lg"
              aria-label="Previous folder page"
            >
              <ChevronLeft size={15} />
            </button>
            <span className="folder-page-count inline-flex min-h-10 items-center rounded-lg px-3 font-mono text-[11px] text-frost-dim">
              {folderPage + 1} / {totalFolderPages}
            </span>
            <button
              type="button"
              onClick={() => goToFolderPage(folderPage + 1)}
              disabled={folderPage >= totalFolderPages - 1}
              className="folder-page-button inline-flex min-h-10 min-w-10 items-center justify-center rounded-lg"
