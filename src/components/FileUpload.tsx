...
export function FileUpload({ onUploadComplete, onOpenRecovery }: FileUploadProps) {
  ...
  if (!connected) {
    return (
      <div className="premium-surface flex min-h-[300px] flex-col items-center justify-center rounded-xl border-dashed px-6 py-12 text-center md:px-10">
        <div className="accent-chip mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl">
          <Lock size={20} />
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`security-mode-card flex items-center justify-between gap-4 rounded-2xl px-4 py-4 ${encrypt ? "is-encrypted" : ""}`}>
      ...
      <div
        ...
        className={`dropzone-shell relative flex min-h-[260px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-xl border border-dashed px-6 py-12 text-center transition-all duration-300 md:px-10 ${isDragging ? "is-active drop-zone-active" : "premium-surface"}`}
        style={{
      ...
      >
        ...
        <p className={`mb-1 font-display text-lg font-semibold tracking-[-0.02em] ${isDragging ? "text-[var(--acid)]" : "text-frost"}`}>
          {isDragging ? "Drop to add files" : "Drop files to seal"}
        </p>
        <p className="font-mono text-xs text-frost-muted">
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          {files.map((f) => (
            <div key={f.id}
              className="premium-surface flex items-center gap-3 rounded-xl px-4 py-3 transition-colors"
            >
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg border border-[var(--surface-border)] bg-[var(--surface-muted)]">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(f.status === "uploading" || f.status === "encrypting") && (
                  <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--acid-glow)]">
      ...
      {pendingCount > 0 && (
        <button
          ...
          className="premium-button flex min-h-12 w-full items-center justify-center gap-2 rounded-xl px-4 py-3 font-display text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-acid/50 focus:ring-offset-2 focus:ring-offset-obsidian-950"
        >
          {isUploading ? (
            <>
      ...
      {receipts.length > 0 && recoveryStatus.type !== "saved" && (
        <div className="recovery-reminder rounded-2xl border border-[var(--surface-border-strong)] bg-[var(--acid-glow)] p-4">
...
function UploadPreflightPanel({ summary }: { summary: UploadPreflightSummary }) {
  return (
    <section className={`preflight-panel premium-surface rounded-2xl p-4 ${summary.status}`}>
...
function UploadPipeline({ files }: { files: UploadFile[] }) {
  ...
  return (
    <div className="pipeline-shell premium-surface rounded-xl px-4 py-4">
