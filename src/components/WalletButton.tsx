...
const asWalletOption = (wallet: unknown): WalletOption => wallet as WalletOption;

export function WalletButton() {
  const { connect, disconnect, connected, account, wallet, wallets, notDetectedWallets } = useWallet();
  const [copied, setCopied] = useState(false);
  const [open, setOpen] = useState(false);
  const currentWallet = wallet as unknown as WalletOption | null;

  const walletOptions = [...wallets, ...notDetectedWallets]
    .map(asWalletOption)
    .filter(
        ...
        {open && (
          <div className="wallet-menu absolute right-0 top-full z-50 mt-3 w-60 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface)] backdrop-blur-xl animate-fade-in">
            <div className="border-b border-[var(--surface-border)] px-4 py-4">
              ...
              <p className="font-mono text-xs text-frost">{currentWallet?.name || "Wallet"}</p>
            </div>
            <button
              type="button"
              onClick={handleCopy}
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => setOpen(!open)}
        aria-expanded={open}
        className="premium-button flex min-h-12 items-center justify-center gap-2 whitespace-nowrap rounded-xl px-5 py-2 font-display text-base font-semibold shadow-[0_18px_40px_-28px_rgba(156,206,118,0.75)] focus:outline-none focus:ring-2 focus:ring-acid/50 focus:ring-offset-2 focus:ring-offset-obsidian-950"
      >
        <Wallet size={15} />
      ...
      {open && (
        <div className="wallet-menu absolute right-0 top-full z-50 mt-3 w-[min(92vw,380px)] overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface)] backdrop-blur-xl animate-fade-in">
          <div className="border-b border-[var(--surface-border)] px-5 py-5">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--acid)]">connect wallet</p>
