...
function RecoveryGuide({
  ...
  return (
    <div className="mt-5 grid gap-3 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
      <div>
...
function Metric({ label, value, loading = false }: { label: string; value: string; loading?: boolean }) {
  return (
    <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-4 py-3">
      <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-frost-muted">{label}</p>
      {loading ? (
        <div className="mt-2 h-4 w-28 animate-pulse rounded bg-[var(--surface-border)]" />
      ) : (
