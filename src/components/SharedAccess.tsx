...
export function SharedAccess() {
  ...
  return (
    <div className="shared-access-page grid gap-5">
      <section className="shared-import-panel premium-surface rounded-2xl p-4 md:p-5">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
          <div>
            ...
            <p className="mt-1 max-w-[64ch] text-sm leading-6 text-frost-muted">
              Paste the owner-signed grant JSON. The access code stays separate until you unlock the file.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3">
          <div className="shared-flow-steps grid gap-2 sm:grid-cols-3">
            <div className="shared-flow-step">
              <span>01</span>
              <p>Import JSON</p>
            </div>
            <div className="shared-flow-step">
          ...
          <div className="shared-info-panel rounded-xl border border-[var(--surface-border)] px-4 py-3">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="shared-info-cell">
          ...
          <textarea
            ...
            placeholder='Paste the BlobSafe grant JSON here. Do not paste the access code into this field.'
            className="shared-json-input min-h-[150px] resize-y rounded-xl border border-[var(--surface-border)] bg-[var(--surface-raised)] px-4 py-3 font-mono text-xs leading-6 text-frost outline-none transition-colors placeholder:text-frost-muted focus:border-[var(--surface-border-strong)] focus:ring-2 focus:ring-acid/20"
          />
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              ...
              disabled={!connected || !grantJson.trim()}
              className="premium-button inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 font-display text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-acid/40 disabled:cursor-not-allowed disabled:opacity-60"
            >
              <Upload size={14} />
              Import grant
            </button>
      ...
      {grants.length === 0 ? (
        <div className="shared-empty-state empty-state-premium flex min-h-[240px] flex-col items-center justify-center rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-6 py-12 text-center">
        ...
        <div className="shared-grants-grid grid gap-3">
          {grants.map((grant) => (
            <SharedGrantCard
              key={grant.id}
              grant={grant}
...
function SharedGrantCard({
  ...
  return (
    <section className="shared-grant-card premium-surface rounded-2xl p-4 md:p-5">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
        <div className="shared-grant-main min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`shared-status-pill inline-flex items-center gap-1 rounded-md border px-2 py-1 font-mono text-[10px] uppercase tracking-[0.1em] ${statusClass}`}>
          ...
          <div className="shared-grant-meta mt-4 flex flex-wrap gap-x-3 gap-y-1 font-mono text-[11px] uppercase tracking-[0.08em] text-frost-muted">
          ...
          <div className="shared-grant-status mt-3 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-3 py-2">
        ...
        <div className="shared-unlock-panel grid gap-2">
          <div className="mb-1">
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-frost-muted">access code</p>
            <p className="mt-1 text-xs leading-5 text-frost-muted">
              Enter the code sent separately by the owner.
            </p>
          </div>
          <input
            value={accessCode}
            onChange={(event) => onAccessCodeChange(event.target.value)}
            placeholder="proof-seal-10bvbn"
