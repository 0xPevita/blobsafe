...
export function explainError(error: unknown, context: ErrorContext = "upload"): RecoveryHint {
  ...
  if (matches(message, ["shelby rpc request timed out", "storage request timed out"])) {
    return {
      title: "Shelby storage timed out",
      message: "The on-chain registration finished, but the Shelby storage endpoint did not accept the file bytes in time.",
      action: "Refresh Files first. If the file is not visible, retry the same file once with the same network selected.",
    };
  }
